...
2. **Getting Here** — from SFO (BART), from OAK, driving/parking (1 driveway space + free street), step-by-step keypad entry with photos, luggage note (narrow alley), early-arrival policy
3. **The Cottage** — room-by-room (living/sofa-bed topper tip, BR1, loft BR2 low-ceiling warning, kitchen, bathroom, laundry, shared courtyard/firepit); appliance how-tos (TVs/Netflix, Keurig vs drip, Sodastream, dishwasher, washer/dryer, heating + portable fans, no A/C); house rules; trash/recycling/compost (SF three-bin rules + pickup day); troubleshooting/FAQ (breaker, water shutoff, WiFi reboot, packages)
4. **Getting Around** — inline map (Leaflet/OSM) with transit pins & walk times; K/BART/J/M/buses with live departures; Clipper via Apple/Google Pay explainer; Muni vs BART primer; rideshare/taxi notes; app recommendations
5. **Eat & Drink** — curated walkable list (Philz, Beeps, Pakwan, Sakesan, etc.) with map pins, walk times, hours, Keith's one-line takes; delivery options (DoorDash/Uber Eats/Instacart)
6. **Groceries & Essentials** — Whole Foods 4 min, Safeway, Walgreens, Stonestown, Serramonte; pharmacy; ATM
7. **Explore SF** — curated sights with transit directions from the cottage; **interest-based suggestions** instead of fixed itineraries: *Into hiking/nature?* (Lands End, Twin Peaks, Ocean Beach…), *Have a car?* (Muir Woods, Half Moon Bay, wine country), *Wine tasting?*, *Museums?*, *With kids?* — tagged so guests filter by interest
8. **Work From Cottage** — desks, 817 Mbps WiFi, video-call tips, cafes to work from
9. **Kids & Pets** — pack-and-play, playgrounds/parks, SF Zoo nearby, vets, pet stores, dog-friendly spots
10. **SF Tips & Safety** — layers/fog weather reality, car break-in warning (never leave items visible), earthquake basics, tipping culture, neighborhood context
11. **What's On** — seasonal events calendar; windowed to stay dates when hash present
12. **Emergency** — 911/non-emergency/311, nearest urgent care & ER, fire extinguisher/first-aid locations, host contact
13. **Checkout** — time, checklist, review nudge, book-direct pitch, guide feedback form
